...
; V.1.8 Beta
; - Fx-Befehle 861->870, 870->880
; - neuer Fx-Befehl 89n zur Auswahl des Audiokanals n=0..3 für den Twister
...
; 880	Restart Scrolltext
; 89n	Select audio channel for twister

; COLOR00:	3 x 16	=	48 Farben: Bars, Sprites (2. Color-Bank)
; COLOR00:	2 x 40	=	80 Farben: gestreife Bar
; COLOR01:	3 x 16	=	48 Farben: Bars, Sprites (2. Color-Bank)
; COLOR01:	1 x 16 	=	16 Farben: Farbverlauf Scrolltext zentriert

; Ausführungszeit 68020: ? Rasterzeilen
...
; **** Horiz-Scroll-Logo ****
	...
	move.w	d1,hst_text_delay_counter(a3) ; Zähler inaktiv
...
; **** Bar-Fader-In ****
	...
	move.w	#sine_table_length/4,bfi_fader_angle(a3) ; 90 Grad

; **** Bar-Fader-Out ****
	move.w	d1,bfo_active(a3)
	move.w	#sine_table_length/4,bfo_fader_angle(a3) ; 90 Grad
...
; **** Scroll-Logo-Bottom-Out ****
	...
	move.w	#sine_table_length/4,slbo_y_angle(a3) ; 90 Grad
...
horiz_scroll_logo
	...
	swap	d3
	add.w	#hsl_x_center*SHIRES_PIXEL_FACTOR,d3 ; + X-Position
...
tb_get_yz_coords
	...
	add.b	(vm_audio_channel1_info+aci_yanglespeed+1,pc,d1.w*2),d0 ; nächster Y-Winkel
	move.w	d0,tb_y_angle(a3) 
	move.w	d5,d0
	add.b	(vm_audio_channel1_info+aci_yanglestep+1,pc,d1.w*2),d0
...
sp_get_stripes_y_coords
	...
	add.w	sp_stripes_y_angle_speed(a3),d0 ; nächster Y-Winkel
...
tb_set_background_bars_loop1
	...
	ADDF.W	tb_bars_number*LONGWORD_SIZE,a0 ; n x Z-Vektor + Y-Pos. überspringen
...
tb_set_foreground_bars_loop1
	...
	ADDF.W	tb_bars_number*LONGWORD_SIZE,a0 ; Z-Vektor + Y-Pos. überspringen
...
sp_make_color_offsets_table_loop2
	...
	move.w	d1,(a1,d0.w*2)		; Farboffset
...
sp_make_pattern
	...
	moveq	#2<<3,d5		; Additionswert für Farbregisterzähler
...
sp_make_stripe_bar_loop
	...
	bne.s	sp_no_restart_registers_counter
...
horiz_scrolltext_loop
	...
	sub.w	hst_horiz_scroll_speed(a3),d2 ; X-Position verringern
...
hst_horiz_scroll
	...
	move.l	(a0),a0
	ADDF.L	(hst_text_x_position/8)+(hst_text_y_position*pf1_plane_width*pf1_depth3),a0 ; Y-Zentrierung
	WAITBLIT
...
bar_fader_in
	...
	move.w	d2,d0
	ADDF.W	bfi_fader_angle_speed,d0 ; nächster Winkel
...
bar_fader_in_skip
	...
	MOVEF.W bf_colors_number*3,d6	; RGB-Zähler
	lea	sine_table(pc),a0	
	move.l	(a0,d2.w*4),d0		; sin(w)
	...
	move.w	d6,bf_colors_counter(a3) ; Fading-In beendet ?
	bne.s	bar_fader_in_quit
...
bar_fader_out
	...
	move.w	d2,d0
	ADDF.W	bfo_fader_angle_speed,d0 ; nächster Winkel
	cmp.w	#sine_table_length/2,d0	; Y-Winkel <= 180 Grad ?
...
bar_fader_out_skip
	...
	MOVEF.W bf_colors_number-1,d7
	bsr	bf_rgb8_fader_loop
	move.w	d6,bf_colors_counter(a3) ; Fading-Out beendet ?
	bne.s	bar_fader_out_quit
...
bf_convert_colors
	...
	lea	bf_color_cache+(bf_color_table_offset*LONGWORD_SIZE)(pc),a0 ; Puffer für helle Farbwerte
	...
	lea	bf_colors_number*QUADWORD_SIZE(a2),a4 ; Ende der Bar-Farbtabelle
...
slb_scroll_logo
	...
	add.w	d5,d6			; VSTOP
	lea	spr_ptrs_display(pc),a2
	move.w	#spr_x_size2*SHIRES_PIXEL_FACTOR,a4 ; X-Pos.
	moveq	#(spr_used_number/2)-1,d7 ; Anzahl der Attached-Sprites
...
ccfi_fader_mode_2
	...
	neg.w	d0
	addq.w	#BYTE_SIZE,d1		; nächste Spalte
	clr.b	(cl2_display_width-1)-1(a0,d0.w) ; Spaltenstatus: einblenden
	cmp.w	d2,d1			; Alle Spalten eingeblendet ?
	bgt.s	ccfi_fader_mode_skip
	move.w	d1,ccfi_start(a3)
	rts
...
ccfo_fader_mode_2
	...
	neg.w	d0
	addq.w	#BYTE_SIZE,d1		; nächste Spalte
	move.b	#FALSE,(cl2_display_width-1)-1(a0,d0.w) ; Spaltenstatus: ausblenden
	cmp.w	d2,d1			; Alle Spalten ausgeblendet ?
	bgt.s	ccfo_fader_mode_skip
	move.w	d1,ccfo_start(a3)
	rts
...
pt_toggle_stripes_y_movement
	neg.w	sp_stripes_y_angle_speed(a3) ; Richtung umkehren
	rts
	CNOP 0,4
