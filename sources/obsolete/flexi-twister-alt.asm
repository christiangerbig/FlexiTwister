...
; Mit DMA Datafetch 2x funktioniert es !
...
; 830 Start Vert-Scroll-Logo-In
; 840 Start Chunky-Columns-Fader-In
; 850 Restart Scrolltext
...
FMODEBITS                      EQU FMODEF_BPL32+FMODEF_SPR32+FMODEF_SPAGEM
FMODEBITS2                     EQU FMODEF_SPR32+FMODEF_SPAGEM
...
tb_clear_blit_x_size           EQU 16
  IFEQ open_border
tb_clear_blit_y_size           EQU cl2_display_y_size*(cl2_display_width+5)
  ELSE
...
; **** Vert-Scroll-Logo ****
vsl_y_radius                   EQU 264
vsl_y_center                   EQU 264

vsli_y_angle_speed             EQU 2

vslo_y_angle_speed             EQU 1
...
pf1_bitplanes_x_offset         EQU 64
pf1_BPL1DAT_x_offset           EQU 64-16
...
cl2_ext1_WAIT       RS.L 1
  IFEQ open_border
cl2_ext1_FMODE_1    RS.L 1
cl2_ext1_BPL1DAT    RS.L 1
cl2_ext1_NOOP       RS.L 1
cl2_ext1_FMODE_2    RS.L 1
  ENDC
...
; **** Bar-Fader ****
  ...
  move.w  d2,bfi_fader_angle(a3)

  move.w  d1,bfo_state(a3)
  move.w  d2,bfo_fader_angle(a3)

; **** Vert-Scroll-Logo ****
  move.w  d1,vsli_state(a3)
  move.w  d0,vsli_y_angle(a3) ;0 Grad

  move.w  d1,vslo_state(a3)
  moveq   #(sine_table_length/4)-1,d2 ;90 Grad
  move.w  d2,vslo_y_angle(a3)
...
; **** Chunky-Columns-Fader ****
  ...
  moveq   #ccfi_delay_speed,d2
  move.w  d2,ccfi_columns_delay_counter(a3)

  move.w  d1,ccfo_state(a3)
  moveq   #ccfo_mode1,d2
  move.w  d2,ccfo_current_mode(a3)
  move.w  d0,ccfo_start(a3)
  moveq   #ccfo_delay_speed,d2
  move.w  d2,ccfo_columns_delay_counter(a3)

; **** Main ****
  move.w  d1,intro_state(a3)
  move.w  d1,quit_intro(a3)
  rts
...
; -----------------------------------
  INIT_ATTACHED_SPRITES_CLUSTER lg,spr_pointers_display,,,spr_x_size2,NOHEADER
...
init_second_copperlist
  ...
  COP_INIT_BPLCON4_CHUNKY_SCREEN cl2,cl2_HSTART1,cl2_VSTART1,cl2_display_x_size,cl2_display_y_size,open_border,tb_quick_clear,FALSE,OVERSCAN
...
beam_routines
  ...
  bsr     swap_playfield1
; ** Routines **
  bsr     vert_scroll_logo_in
  bsr     vert_scroll_logo_out
...
; **** Handler ****
  ...
  btst    #2,$dff016
  beq.s   skip
  tst.w   intro_state(a3)    ;Intro beendet ?
...
; ** Logo vertikal einscrollen **
; -------------------------------
  CNOP 0,4
vert_scroll_logo_in
  tst.w   vsli_state(a3)     ;Vert-Scroll-Logo-In an ?
  bne.s   no_vert_scroll_logo_in  ;Nein -> verzweige
  move.w  vsli_y_angle(a3),d2 ;Y-Winkel holen
  cmp.w   #sine_table_length/4,d2 ;90 Grad erreicht ?
  bge.s   no_vert_scroll_logo_in  ;Ja -> verzweige
  move.l  a4,-(a7)
  lea     sine_table,a0      
  move.w  2(a0,d2.w*4),d0    ;cos(w)
  muls.w  #vsl_y_radius*2,d0 ;y'=yr*cos(w)/2^16
  swap    d0
  add.w   #vsl_y_center,d0
  MOVEF.W lg_image_y_position,d5
  add.w   d0,d5              ;Y-Zentrierung
  addq.w  #vsli_y_angle_speed,d2 ;nächster Y-Winkel
  move.w  d2,vsli_y_angle(a3);retten
...
vert_scroll_logo_in_loop
  ...
  dbf     d7,vert_scroll_logo_in_loop
  move.l  (a7)+,a4
no_vert_scroll_logo_in
  rts

; ** Logo vertikal ausscrollen **
; -------------------------------
  CNOP 0,4
vert_scroll_logo_out
  tst.w   vslo_state(a3)     ;Vert-Scroll-Logo-Out an ?
  bne.s   no_vert_scroll_logo_out ;Nein -> verzweige
  move.w  vslo_y_angle(a3),d2 ;Y-Winkel holen
  bmi.s   no_vert_scroll_logo_out ;Ja -> verzweige
  move.l  a4,-(a7)
  lea     sine_table,a0      
  move.w  2(a0,d2.w*4),d0    ;cos(w)
  muls.w  #vsl_y_radius*2,d0 ;y'=yr*cos(w)/2^16
  ...
  MOVEF.W lg_image_y_position,d5
  add.w   d0,d5              ;Y-Zentrierung
  subq.w  #vsli_y_angle_speed,d2 ;nächster Y-Winkel
  move.w  d2,vslo_y_angle(a3);retten
  MOVEF.W lg_image_x_position*4,d4 ;X
  MOVEF.W lg_image_y_size,d6 ;Höhe
  add.w   d5,d6              ;Höhe zu Y addieren
  lea     spr_pointers_display(pc),a2 ;Zeiger auf Sprites
...
vert_scroll_logo_out_loop
  ...
  tas     d2                 ;Attached-Bit setzen
  move.w  d2,spr_pixel_per_datafetch/8(a1) ;SPRCTL
  dbf     d7,vert_scroll_logo_out_loop
  move.l  (a7)+,a4
no_vert_scroll_logo_out
  rts
...
cl2_update_BPL1DAT
  ...
  MOVEF.W FMODEBITS,d3
  MOVEF.W FMODEBITS2,d4
...
cl2_update_BPL1DAT_loop
  ...
  move.w  d4,-4(a1)          ;1x Bitplane-Datafetch
  add.l   d2,a1              ;nächste Zeile in CL
  move.w  d3,8-cl2_extension1_SIZE(a1) ;2x Bitplane-Datafetch
...
chunky_columns_fader_in
  ...
  beq.s   ccfi_no_chunky_columns_fader_in ;Ja -> verzweige
  subq.w  #ccfi_delay_speed,ccfi_columns_delay_counter(a3) ;Verzögerungszähler herunterzählen
  bne.s   ccfi_no_chunky_columns_fader_in ;Wenn nicht NULL -> verzweige
...
chunky_columns_fader_out
  ...
  tst.w   ccfi_state(a3)     ;Chunky-Columns-Fader-In an ?
  beq.s   ccfo_no_chunky_columns_fader_out ;Ja -> verzweige
  subq.w  #ccfo_delay_speed,ccfo_columns_delay_counter(a3) ;Verzögerungszähler herunterzählen
  bne.s   ccfo_no_chunky_columns_fader_out ;Wenn nicht NULL -> verzweige
...
hst_stop_scrolltext
  ...
  tst.w   quit_intro(a3)     ;Soll Intro beendet werden?
...
hst_quit_and_stop_scrolltext
  ...
  cmp.w   #sine_table_length/4,vsli_y_angle(a3) ;90 Grad erreicht ?
  blt.s   hst_no_vert_scroll_logo_out  ;Ja -> verzweige
  move.w  d0,vslo_state(a3)  ;Vert-Scroll-Logo-Out an
hst_no_vert_scroll_logo_out
  move.w  d0,ccfo_state(a3)  ;Chunky-Columns-Fader-Out an
  move.w  #ssb_bar_height*3,bf_colors_counter(a3)
...
bf_convert_color_table
  ...
  lea     (ssb_bar_height/2)*LONGWORDSIZE(a0),a1 ;Puffer für dunkle Farbwerte
  move.l  extra_memory(a3),a2
  add.l   #em_color_table,a2 ;Zeiger auf Farbtabelle
  lea     ssb_bar_height*LONGWORDSIZE*2(a2),a4 ;Ziel: Ende der Bar-Farbtabelle
  MOVEF.W (ssb_bar_height/2)-1,d7 ;Anzahl der Farben
...
bfi_no_restart_fader_angle
  ...
  MOVEF.W ssb_bar_height*3,d6 ;Zähler
  ...
  MOVEF.W ssb_bar_height-1,d7 ;Anzahl der Farben
  bsr     bf_fader_loop
  movem.l (a7)+,a4-a6
...
mouse_handler
  ...
  beq.s   mh_quit_intro      ;Ja -> verzweige
  rts
  CNOP 0,4
mh_quit_intro
  moveq   #FALSE,d0
...
mh_quit_without_scrolltext
  move.w  d0,pt_fade_out_music_state(a3) ;Musik ausfaden
  cmp.w   #sine_table_length/4,vsli_y_angle(a3) ;90 Grad erreicht ?
  blt.s   mh_no_vert_scroll_logo_out ;Ja -> verzweige
  move.w  d0,vslo_state(a3)  ;Vert-Scroll-Logo-Out an
mh_no_vert_scroll_logo_out
  move.w  d0,ccfo_state(a3)  ;Chnky-Columns-Fader-Out an
  move.w  #ssb_bar_height*3,bf_colors_counter(a3)
  move.w  d0,bfo_state(a3)   ;Bar-Fader-Out an
  move.w  d0,bf_convert_colors_state(a3) ;Konvertieren der Farben an
  rts
...
mh_quit_with_scrolltext
  ...
  move.w  d0,quit_intro(a3)  ;Intro soll nach Text-Stopp beendet werden
  rts
...
pt_trigger_fx
  ...
  cmp.b   #$30,d0
  beq.s   pt_start_vert_scroll_logo_in
  cmp.b   #$40,d0
...
pt_start_scrolltext
  ...
  rts
  CNOP 0,4
pt_start_vert_scroll_logo_in
  clr.w   vsli_state(a3)     ;Vert-Scroll-Logo-In an
  rts
  CNOP 0,4
...
hst_restart_text
  ...
  DC.B "GREETINGS TO  -ALL RESISTANCE MEMBERS-  -BIFAT-  -JASMIN68K-  -TONI WILEN-  -BYTEBREAKER-           "

  REPT (hst_text_characters_number)/(hst_origin_character_x_size/hst_text_character_x_size)
    DC.B " "
  ENDR

  DC.B "YES!  THIS IS PURE RASTERMANIA!  CHECKOUT THE SPRITES!  THE LOGO IS A CLUSTER OF ALL EIGHT OF THEM DISPLAYED ON AN OVERSCAN DISPLAY!           "
...
; ----------------------------------------
prg_version DC.B "$VER: dissident_flexi-twister 1.0 beta (2.2.24)",TRUE
  EVEN
...
lg_image_data SECTION lg_gfx,DATA
  INCBIN "Daten:Asm-Sources.AGA/FlexiTwister/graphics/256x54x8x2-Dissident.rawblit"
